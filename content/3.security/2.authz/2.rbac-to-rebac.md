...
The reason this order matters is the [same consistency concern from the Zanzibar post](/blog/security/authz/zanzibar-demystified-rebac-openfga): a check must never be staler than the data it guards. During backfill, tuples are still being written, so keep SQL authoritative until the tuple set is provably complete. Flip only when fresh.
