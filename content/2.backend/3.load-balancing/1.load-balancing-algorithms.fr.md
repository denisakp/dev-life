...
Cela boucle avec le reste de la boite a outils de resilience. L'ejection par health check est le cousin cote selection-de-serveur du [circuit breaker qu'un client utilise pour cesser d'appeler une dependance morte](/blog/backend/api-design/retry-storms), et il se tient a cote du [rate limiting qui deleste la charge qu'un service ne peut servir](/blog/backend/api-design/rate-limiting-at-scale). Le load balancing est la troisieme face de la meme piece : le rate limiting decide *combien* un serveur prend, les retries decident *quand* un client envoie, et le load balancing decide *quel* serveur le recoit. Rendez la selection parfaite et sautez les health checks, et vous avez construit une facon tres equitable de repartir la panne.
