...
- We define three volumes: `mongo1_data`, `mongo2_data`, and `mongo3_data` to persist the data for each MongoDB
  container.
- We define three services: `mongo1`, `mongo2`, and `mongo3`. Each service runs a MongoDB container with the
  `mongo:8.2-noble` image. We specify the `--replSet` option to set the replica set name to `rs0`. We also specify the
...
#### Process the replicaset connection
