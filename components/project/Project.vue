  ...
  <div class="h-full flex flex-col slick-border p-6 rounded-sm">
    <div class="flex-grow">
      ...
      <div class="flex flex-grow flex-wrap space-x-2 mt-2">
        <p
          ...
          :key="_index"
          class="mt-1 text-xs py-1 px-2 bg-dark-low slick-border rounded-sm dark-text"
        >
          {{ tag }}
        </p>
      </div>
    ...
    <div class="flex">
      <div v-if="project.github" class="mt-6">
        <nuxt-link :to="project.github" target="_blank">
          <button
      ...
      <div v-if="project.preview" class="mt-6 ml-2">
        <nuxt-link :to="project.preview" target="_blank">
          <button
            class="px-3 py-2 sm:px-4 sm:py-3 text-sm sm:text-base font-medium flex items-center justify-center transition duration-500 ease-in-out text-dark-high focus:outline-none focus:shadow-outline bg-green rounded-full"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
